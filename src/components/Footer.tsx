...
const Footer = () => {
  ...
  const footerLinkClasses = "text-muted-foreground hover:text-primary transition-colors duration-200 text-sm";
  const socialLinkClasses = "text-muted-foreground hover:text-primary transition-colors duration-200 flex items-center space-x-2";
  const headingClasses = "text-lg font-semibold mb-5 text-foreground";

  return (
    <footer className="bg-background border-t border-border py-16 px-6 md:px-10">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-16">
          <div className="md:col-span-2 lg:col-span-2">
            <Link to="/" className="text-3xl font-heading font-bold mb-4 inline-block text-foreground">
              ...
              {isAdmin && (
                <>
                  <li className="pt-4 mt-4 border-t border-border">
                    <p className="text-primary font-semibold text-sm">Bảng điều khiển Admin</p>
                  </li>
        ...
        <div className="border-t border-border mt-12 pt-8 flex flex-col md:flex-row justify-between items-center">
          <p className="text-muted-foreground/70 text-sm">© {currentYear} Thi Điểm Cao. Bảo lưu mọi quyền.</p>
          <div className="flex space-x-6 mt-4 md:mt-0">
            <Link to="/privacy-policy" className="text-muted-foreground/70 text-sm hover:text-primary transition-colors">Chính sách Bảo mật</Link>
            <Link to="/terms-of-service" className="text-muted-foreground/70 text-sm hover:text-primary transition-colors">Điều khoản Dịch vụ</Link>
          </div>
        </div>
      </div>
    </footer>

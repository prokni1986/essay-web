...
import Layout from '@/components/Layout'; // Đảm bảo đường dẫn này chính xác

interface Topic {
  _id: string;
  name: string;
  // description?: string; // API hiện không cung cấp
  // image?: string;       // API hiện không cung cấp
}

const TOPICS_PER_PAGE = 6; // Số chủ đề hiển thị trên mỗi trang (ví dụ: 2x3 grid)
...
const AllTopic: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      ...
      try {
        ...
        // setDisplayedTopics(response.data); // Sẽ được xử lý bởi useEffect dưới
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const filtered = allFetchedTopics.filter(topic =>
      ...
      // Nếu API có description: || (topic.description && topic.description.toLowerCase().includes(lowerCaseSearchTerm))
    );
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {topicsForCurrentPage.map((topic) => (
              <Link
                to={`/topic/${topic._id}`} // Sử dụng Link của react-router-dom
                key={topic._id}
                ...
                style={{
                  ...
                  // cursor: "pointer" // Không cần thiết khi dùng Link
                }}
              >
                <div
                  className="w-full h-48 rounded-xl overflow-hidden mb-6 bg-gray-700 flex items-center justify-center" // Placeholder background
                  style={{
                    display: "flex",
                    alignItems: "center",
                ...
                >
                  {/* Nếu API cung cấp 'image', bạn có thể dùng:
                  <img src={topic.image} alt={topic.name} className="object-cover w-full h-full" style={{ minHeight: 150 }} />
                  */}
                ...
                {/* API hiện không cung cấp description. Nếu có, bạn có thể thêm:
                <p className="text-gray-200 mb-5 text-base text-justify w-full h-24 overflow-hidden text-ellipsis"> // Giới hạn chiều cao cho description
